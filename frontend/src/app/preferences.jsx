import React, { useEffect, useMemo, useState } from 'react';
import { PreferencesContext } from './preferences-context';

const STORAGE_KEYS = {
  locale: 'crypto-agent-locale',
  theme: 'crypto-agent-theme',
  selectedSymbol: 'crypto-agent-selected-symbol',
};

const messages = {
  en: {
    brand: 'Crypto Agent',
    publicDashboard: 'Dashboard',
    usage: 'Usage',
    console: 'Console',
    chat: 'Chat',
    config: 'Config',
    history: 'History',
    login: 'Sign In',
    logout: 'Logout',
    openConsole: 'Open Console',
    backToDashboard: 'Public Dashboard',
    switchLanguage: 'Language',
    switchTheme: 'Theme',
    light: 'Light',
    dark: 'Dark',
    loading: 'Loading',
    noData: 'No data',
    save: 'Save',
    delete: 'Delete',
    create: 'Create',
    configured: 'Configured',
    notConfigured: 'Not configured',
    clear: 'Clear',
    cancel: 'Cancel',
    confirm: 'Confirm',
    publicHeadline: 'Public trading performance and runtime visibility',
    publicSubhead: 'Follow strategy analysis, positions, orders, K-line activity, and usage metrics from one responsive dashboard.',
    usageHeadline: 'Usage analytics',
    usageSubhead: 'Inspect token, model, and agent-level consumption without entering the console.',
    consoleHeadline: 'Control console',
    timeframe: 'Timeframe',
    compare: 'Compare',
    symbol: 'Symbol',
    summary: 'Summary',
    analysis: 'Analysis',
    strategyLogic: 'Strategy logic',
    positions: 'Positions',
    pendingOrders: 'Open orders',
    recentOrders: 'Recent orders',
    recentTrades: 'Recent trades',
    dailySummaries: 'Daily summaries',
    historyReview: 'History review',
    shortMemories: 'Short memories',
    newsFlow: 'News flow',
    memoryCenter: 'Memory',
    refresh: 'Refresh',
    equityCompare: 'Equity compare',
    scheduler: 'Scheduler',
    symbolMode: 'Symbol mode',
    symbolFreq: 'Symbol frequency',
    agents: 'Agents',
    totalTokens: '14d tokens',
    totalCost: 'Total cost',
    trackedModels: 'Models',
    trackedAgents: 'Tracked agents',
    totalTrades: 'Total trades',
    totalPnl: 'Total PnL',
    winRate: 'Win rate',
    latestEquity: 'Latest equity',
    latestDay: 'Latest day',
    modelPricing: 'Pricing',
    prompts: 'Prompts',
    runtimeConfig: 'Runtime config',
    globals: 'Global settings',
    agentConfigs: 'Agent configs',
    addAgent: 'Add agent',
    duplicateAgent: 'Duplicate',
    removeAgent: 'Remove',
    deleteAgentData: 'Delete config and linked data',
    basicSettings: 'Basic settings',
    scheduleSettings: 'Schedule and mode',
    modelSettings: 'Decision model',
    exchangeSettings: 'Exchange',
    summarizerSettings: 'Summarizer model',
    advancedSettings: 'Advanced overrides',
    promptEditor: 'Prompt editor',
    pricingEditor: 'Pricing editor',
    saveConfig: 'Save configuration',
    savePrompt: 'Save prompt',
    deletePrompt: 'Delete prompt',
    llmProviders: 'LLM Providers',
    exchangeProfiles: 'Exchange Profiles',
    addProvider: 'Add provider',
    addProfile: 'Add profile',
    decisionModel: 'Decision model',
    primaryDecisionModel: 'Primary Decision Model',
    fallbackChain: 'Fallback Chain',
    fallbackModels: 'Fallback Models',
    addFallbackModel: 'Add Fallback Model',
    fallbackChainTip: 'When the primary model fails (after exhausting globally configured retries), the task automatically falls back to the next model in this chain. Each model retries up to the global limit.',
    noFallbackModels: 'No fallback models configured. Add one below to enable automated model fallback on errors.',
    fallbackStep: 'Fallback #',
    fallbackOnError: 'Fallback on error',
    summarizerModel: 'Summarizer model',
    thinkingMode: 'Thinking mode',
    reasoningEffort: 'Reasoning effort',
    strategyPrompt: 'Strategy summary prompt',
    dailyPrompt: 'Daily summary prompt',
    shortMemoryPrompt: 'Short memory prompt',
    providerName: 'Provider name',
    profileName: 'Profile name',
    selectProvider: 'Select provider',
    selectProfile: 'Select profile',
    noProvider: 'No provider selected',
    noProfile: 'No profile selected',
    createSession: 'New session',
    generateTitle: 'Generate title',
    clearMessages: 'Clear messages',
    deleteSession: 'Delete session',
    historyPageDesc: 'Browse historical trading summaries, compare equity curves, and review agent decisions.',
    chatPageDesc: 'Chat with the selected trading agent. Chat tool calls pause for approval; scheduled agents execute directly.',
    chatPlaceholder: 'Ask about the market, positions, orders, or strategy. Press Enter to send, Shift+Enter for a new line.',
    toolApproval: 'Tool approval required',
    approve: 'Approve',
    reject: 'Reject',
    send: 'Send',
    password: 'Password',
    loginDescription: 'Sign in to manage chat, config, and history.',
    workspace: 'Workspace',
    liveWorkspace: 'Live workspace',
    usageOverview: 'Usage overview',
    modelUsage: 'Model usage',
    agentUsage: 'Agent usage',
    dailyTokens: 'Daily tokens',
    dailyCost: 'Daily cost',
    emptySessions: 'No sessions yet',
    emptyConfig: 'No agent configs yet',
    emptyWorkspace: 'No workspace data available',
    emptyHistory: 'No history data available',
    noActivePositions: 'No active positions',
    noOpenOrders: 'No open orders',
    noSummaries: 'No summaries yet',
    themeLightHint: 'Bright surfaces and soft contrast',
    themeDarkHint: 'Dim surfaces for low-light viewing',
    localeEnHint: 'English labels and content chrome',
    localeZhHint: 'Chinese labels and content chrome',
    setupHeadline: 'Initial setup',
    setupDesc: 'Set bootstrap secrets before using the console in production.',
    dockerDetected: 'Docker runtime detected',
    dockerDesc: 'Environment changes are applied after the container restarts.',
    setupCompleted: 'Setup saved',
    restartBackend: 'Restart the backend or container for security settings to take effect.',
    adminPassword: 'Admin password',
    jwtSecret: 'JWT secret',
    configMasterKey: 'Config master key',
    jwtExpireHours: 'JWT expire hours',
    port: 'Port',
    timezone: 'Timezone',
    runSchedulerInWeb: 'Run scheduler in web process',
    saveSetup: 'Save setup',
    compareView: 'Compare view',
    nextRun: 'Next run',
    executedAt: 'Executed at',
    configColumn: 'Config',
    configIdColumn: 'Config ID',
    modeColumn: 'Mode',
    modelColumn: 'Model',
    promptColumn: 'Prompt',
    completionColumn: 'Completion',
    totalColumn: 'Total',
    costColumn: 'Cost',
    longColumn: 'Long',
    shortColumn: 'Short',
    longShortRatio: 'Long/short',
    closeColumn: 'Close',
    openTotalColumn: 'Total orders',
    cancelColumn: 'Cancel',
    reasoning: 'Reasoning',
    copied: 'Copied',
    saved: 'Saved',
    deleted: 'Deleted',
    edit: 'Edit',
    confirmDelete: 'Delete this item?',
    confirmDeleteSession: 'Delete this session and all messages?',
    deleteSelected: 'Delete selected',
    deleteFiltered: 'Delete filtered',
    addDailySummary: 'Add daily summary',
    editDailySummary: 'Edit daily summary',
    generateDailySummary: 'Generate summary',
    time: 'Time',
    action: 'Action',
    entry: 'Entry',
    amount: 'Amount',
    status: 'Status',
    note: 'Note',
    reason: 'Reason',
    side: 'Side',
    type: 'Type',
    price: 'Price',
    orderId: 'Order ID',
    rawType: 'Raw type',
    triggerPrice: 'Trigger price',
    posSide: 'Position side',
    mark: 'Mark',
    qty: 'Qty',
    unrealizedPnl: 'Unrealized PnL',
    roiPct: 'ROI %',
    leverage: 'Leverage',
    avgCost: 'Avg cost',
    spotAccount: 'Spot account',
    marketValue: 'Market value',
    totalInvested: 'Total invested',
    actualBalance: 'Exchange balance',
    recordedQty: 'Recorded quantity',
    buyCount: 'Buy count',
    lastSync: 'Last sync',
    equity: 'Equity',
    marginBalance: 'Margin balance',
    walletBalance: 'Wallet balance',
    date: 'Date',
    sources: 'Sources',
    created: 'Created',
    bucket: 'Bucket',
    end: 'End',
    marketDecision: 'Market / Decision',
    position: 'Position',
    takeProfit: 'Take Profit',
    stopLoss: 'Stop Loss',
    adjustTpSl: 'Adjust TP / SL',
    takeProfitPrice: 'Take Profit Price',
    stopLossPrice: 'Stop Loss Price',
    clearTp: 'Clear Take Profit',
    clearSl: 'Clear Stop Loss',
    tpSlNotice: 'Adjusting or clearing TP/SL synchronizes exchange orders and updates the internal protection plan without being replenished by the background scheduler.',
  },
  zh: {
    brand: 'Crypto Agent',
    publicDashboard: '公开看板',
    usage: '用量统计',
    console: '控制台',
    chat: '聊天',
    config: '配置',
    history: '历史',
    login: '登录',
    logout: '退出',
    openConsole: '进入控制台',
    backToDashboard: '返回公开看板',
    switchLanguage: '语言',
    switchTheme: '主题',
    light: '亮色',
    dark: '暗色',
    loading: '加载中',
    noData: '暂无数据',
    save: '保存',
    delete: '删除',
    create: '创建',
    configured: '已配置',
    notConfigured: '未配置',
    clear: '清空',
    cancel: '取消',
    confirm: '确认',
    publicHeadline: '公开展示交易表现与运行状态',
    publicSubhead: '在一个响应式看板里查看策略分析、持仓、订单、K 线和用量指标。',
    usageHeadline: '用量分析',
    usageSubhead: '无需进入控制台，也可以查看 token、模型与 agent 的消耗情况。',
    consoleHeadline: '控制台',
    timeframe: '周期',
    compare: '对比',
    symbol: '交易对',
    summary: '摘要',
    analysis: '分析内容',
    strategyLogic: '策略逻辑',
    positions: '持仓',
    pendingOrders: '当前挂单',
    recentOrders: '最近订单',
    recentTrades: '最近成交',
    dailySummaries: '每日总结',
    historyReview: '历史回溯',
    shortMemories: '短期记忆',
    newsFlow: '消息面',
    memoryCenter: '记忆管理',
    refresh: '刷新',
    equityCompare: '权益对比',
    scheduler: '调度器',
    symbolMode: '运行模式',
    symbolFreq: '运行频率',
    agents: 'Agent 数',
    totalTokens: '14 天 Tokens',
    totalCost: '总成本',
    trackedModels: '模型数',
    trackedAgents: 'Agent 数',
    totalTrades: '总交易数',
    totalPnl: '总盈亏',
    winRate: '胜率',
    latestEquity: '最新权益',
    latestDay: '最近一天',
    modelPricing: '计价',
    prompts: 'Prompt',
    runtimeConfig: '运行配置',
    globals: '全局设置',
    agentConfigs: 'Agent 配置',
    addAgent: '新增 Agent',
    duplicateAgent: '复制',
    removeAgent: '移除',
    deleteAgentData: '删除配置并清理关联数据',
    basicSettings: '基础信息',
    scheduleSettings: '调度与模式',
    modelSettings: '决策模型',
    exchangeSettings: '交易所',
    summarizerSettings: '总结模型',
    advancedSettings: '高级覆盖项',
    promptEditor: 'Prompt 编辑',
    pricingEditor: '计价编辑',
    saveConfig: '保存配置',
    savePrompt: '保存 Prompt',
    deletePrompt: '删除 Prompt',
    llmProviders: '模型服务商',
    exchangeProfiles: '交易所配置',
    addProvider: '新增服务商',
    addProfile: '新增交易所配置',
    decisionModel: '决策模型',
    primaryDecisionModel: '主决策模型',
    fallbackChain: '链路兜底',
    fallbackModels: '兜底模型链路',
    addFallbackModel: '添加兜底模型',
    fallbackChainTip: '当主模型请求报错失败（试错全局设置的次数后），系统将自动按链路顺序使用下一个设置的模型进行兜底执行；每个请求都试错全局设置的次数。',
    noFallbackModels: '暂未配置兜底模型。点击下方按钮添加兜底模型，当主模型报错时自动切换。',
    fallbackStep: '兜底模型 #',
    fallbackOnError: '报错后兜底',
    summarizerModel: '总结模型',
    thinkingMode: '思考模式',
    reasoningEffort: '推理力度',
    strategyPrompt: '策略总结 Prompt',
    dailyPrompt: '每日总结 Prompt',
    shortMemoryPrompt: '短期记忆 Prompt',
    providerName: '服务商名称',
    profileName: '配置名称',
    selectProvider: '选择服务商',
    selectProfile: '选择交易所配置',
    noProvider: '未选择服务商',
    noProfile: '未选择交易所配置',
    importExport: '导入导出',
    createSession: '新建会话',
    generateTitle: '生成标题',
    clearMessages: '清空消息',
    deleteSession: '删除会话',
    historyPageDesc: '查看历史交易总结，对比权益曲线，回溯 Agent 决策过程。',
    chatPageDesc: '和选中的交易 Agent 对话；只有聊天里的工具调用会先暂停等待确认，定时 Agent 会直接执行。',
    chatPlaceholder: '询问行情、持仓、挂单或策略。按 Enter 发送，Shift+Enter 换行。',
    toolApproval: '工具调用待审批',
    approve: '批准',
    reject: '拒绝',
    send: '发送',
    password: '密码',
    loginDescription: '登录后可管理聊天、配置和历史。',
    workspace: '工作区',
    liveWorkspace: '实时工作区',
    usageOverview: '用量概览',
    modelUsage: '模型用量',
    agentUsage: 'Agent 用量',
    dailyTokens: '每日 Token',
    dailyCost: '每日成本',
    emptySessions: '暂无会话',
    emptyConfig: '暂无 Agent 配置',
    emptyWorkspace: '暂无工作区数据',
    emptyHistory: '暂无历史数据',
    noActivePositions: '暂无持仓',
    noOpenOrders: '暂无挂单',
    noSummaries: '暂无总结',
    compareView: '对比视图',
    nextRun: '下次运行',
    executedAt: '执行时间',
    configColumn: '配置',
    configIdColumn: '配置 ID',
    modeColumn: '模式',
    modelColumn: '模型',
    promptColumn: 'Prompt',
    completionColumn: 'Completion',
    totalColumn: '总计',
    costColumn: '成本',
    longColumn: '做多',
    shortColumn: '做空',
    longShortRatio: '多空比',
    closeColumn: '平仓',
    openTotalColumn: '订单总数',
    cancelColumn: '撤单',
    reasoning: '推理过程',
    copied: '已复制',
    saved: '已保存',
    deleted: '已删除',
    edit: '编辑',
    confirmDelete: '确认删除？',
    confirmDeleteSession: '确认删除这个会话和所有消息？',
    deleteSelected: '删除选中',
    deleteFiltered: '按筛选删除',
    addDailySummary: '新增每日总结',
    editDailySummary: '编辑每日总结',
    generateDailySummary: '生成总结',
    time: '时间',
    action: '动作',
    entry: '入场',
    amount: '数量',
    status: '状态',
    note: '备注',
    reason: '原因',
    side: '方向',
    type: '类型',
    price: '价格',
    orderId: '订单 ID',
    rawType: '原始类型',
    triggerPrice: '触发价',
    posSide: '持仓方向',
    mark: '标记价',
    qty: '数量',
    unrealizedPnl: '未实现盈亏',
    roiPct: 'ROI %',
    leverage: '杠杆',
    avgCost: '平均成本',
    spotAccount: '现货账户',
    marketValue: '当前市值',
    totalInvested: '累计投入',
    actualBalance: '交易所余额',
    recordedQty: '记录数量',
    buyCount: '买入次数',
    lastSync: '最后同步',
    equity: '权益',
    marginBalance: '保证金余额',
    walletBalance: '钱包余额',
    date: '日期',
    sources: '来源',
    created: '创建时间',
    bucket: '时间段',
    end: '结束',
    marketDecision: '市场 / 决策',
    position: '持仓',
    takeProfit: '止盈价',
    stopLoss: '止损价',
    adjustTpSl: '调整止盈止损',
    takeProfitPrice: '止盈价格',
    stopLossPrice: '止损价格',
    clearTp: '取消/清空止盈',
    clearSl: '取消/清空止损',
    tpSlNotice: '修改或清空止盈止损将同步更新交易所挂单及内部保护计划，后台调度器不会自动补挂。',
    themeLightHint: '明亮背景与柔和对比',
    themeDarkHint: '低光环境下更舒适',
    localeEnHint: '界面标签使用英文',
    localeZhHint: '界面标签使用中文',
  },
};

function detectLocale() {
  if (typeof window === 'undefined') {
    return 'en';
  }
  const saved = window.localStorage.getItem(STORAGE_KEYS.locale);
  if (saved === 'en' || saved === 'zh') {
    return saved;
  }
  return navigator.language.toLowerCase().startsWith('zh') ? 'zh' : 'en';
}
function detectTheme() {
  if (typeof window === 'undefined') {
    return 'light';
  }
  const saved = window.localStorage.getItem(STORAGE_KEYS.theme);
  if (saved === 'light' || saved === 'dark') {
    return saved;
  }
  return window.matchMedia?.('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
}

function detectSelectedSymbol() {
  if (typeof window === 'undefined') {
    return '';
  }
  return window.localStorage.getItem(STORAGE_KEYS.selectedSymbol) || '';
}

export function PreferencesProvider({ children }) {
  const [locale, setLocale] = useState(detectLocale);
  const [theme, setTheme] = useState(detectTheme);
  const [selectedSymbol, setSelectedSymbol] = useState(detectSelectedSymbol);

  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEYS.locale, locale);
    document.documentElement.lang = locale === 'zh' ? 'zh-CN' : 'en';
  }, [locale]);

  useEffect(() => {
    window.localStorage.setItem(STORAGE_KEYS.theme, theme);
    document.documentElement.dataset.theme = theme;
  }, [theme]);

  useEffect(() => {
    if (selectedSymbol) {
      window.localStorage.setItem(STORAGE_KEYS.selectedSymbol, selectedSymbol);
    } else {
      window.localStorage.removeItem(STORAGE_KEYS.selectedSymbol);
    }
  }, [selectedSymbol]);

  const value = useMemo(() => {
    const dictionary = messages[locale] || messages.en;
    return {
      locale,
      setLocale,
      theme,
      setTheme,
      selectedSymbol,
      setSelectedSymbol,
      isDark: theme === 'dark',
      t(key) {
        return dictionary[key] || messages.en[key] || key;
      },
    };
  }, [locale, theme, selectedSymbol]);

  return <PreferencesContext.Provider value={value}>{children}</PreferencesContext.Provider>;
}
