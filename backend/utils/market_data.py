...
class MarketTool:
    ...
    def run_silent_sl_tp(self):
        ...
        if not self.config_id or not self.symbol:
            return result
        try:
            # 先清理过期未成交挂单，避免一直占用 OPEN 状态
            try:
                expired = database.auto_close_expired_mock_orders(
                    config_id=self.config_id, symbol=self.symbol
                )
                result["expired"] = int(expired or 0)
                if expired:
                    logger.info(
                        f"🧹 [SilentMonitor] {self.config_id} {self.symbol} 自动取消过期未成交挂单: {expired}"
                    )
            except Exception as exc:
                logger.warning(
                    f"⚠️ [SilentMonitor] 清理过期挂单失败 {self.config_id} {self.symbol}: {exc}"
                )
            ...
            if not active_orders:
                logger.debug(
                    f"[SilentMonitor] {self.config_id} {self.symbol} 无挂单/持仓，跳过检测"
                )
                result["success"] = True
                return result
            ...
            if ohlcv:
                ...
                l = float(ohlcv[0][3])
                before_snapshot = {
                    o["order_id"]: int(o.get("is_filled") or 0) for o in active_orders
                }
                self._check_mock_orders_tp_sl(self.symbol, h, l, candle_ts_ms)

                # 重新拉取查看哪些被标记入场或平仓
                after = database.get_mock_orders(
                    symbol=self.symbol, config_id=self.config_id
                )
                after_map = {o["order_id"]: int(o.get("is_filled") or 0) for o in after}
                filled_count = 0
                for oid, was_filled in before_snapshot.items():
                    now_filled = after_map.get(oid, was_filled)
                    if not was_filled and now_filled:
                        filled_count += 1
                # closed = before active - still active - filled_newly (approximation)
                closed_count = max(len(before_snapshot) - len(after_map), 0)
                result["filled"] = filled_count
                result["closed"] = closed_count
                result["success"] = True
                if filled_count or closed_count:
                    logger.info(
                        f"⚡ [SilentMonitor] {self.config_id} {self.symbol} "
                        f"检测完成: 已入场新增={filled_count}, 平仓数={closed_count}, "
                        f"活跃挂单={len(active_orders)}"
                    )
                else:
                    logger.debug(
                        f"[SilentMonitor] {self.config_id} {self.symbol} 检测完成: 无触发"
                    )
            else:
                logger.warning(
                    f"⚠️ [SilentMonitor] {self.config_id} {self.symbol} 获取 1m K线失败"
                )
        except Exception as e:
            result["error"] = str(e)
