...
DB_NAME = Path(os.getenv("TRADING_DB_PATH", DATA_DIR / "trading_data.db"))
...
def runtime_options_payload() -> dict[str, Any]:
    return {
        ...
        "dca_freqs": ["1d", "1w"],
        "market_timeframes": ["15m", "30m", "1h", "4h", "1d", "1w", "1M", "1m", "5m"],
...
def import_full_snapshot(
    ...
    if model_pricing:
        ...
        if pricing_restored:
            try:
                from backend.database import _write_pricing_file_from_db
                _write_pricing_file_from_db()
            except Exception as e:
                logger.warning(f"⚠️ pricing.json 同步失败: {e}")
