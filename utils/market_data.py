...
class MarketTool:
    ...
    def process_timeframe(self, symbol, tf):
        ...
        try:
            ...
            if e20_val > e50_val > e200_val:
                trend_status = "Uptrend"
            elif e20_val < e50_val < e200_val:
            ...
            return {
                ...
                "volume_analysis": {
                    ...
                    "status": "High" if float(vol_ratio.iloc[-1]) > 1.5 else (
                        "Low" if float(vol_ratio.iloc[-1]) < 0.5 else "Normal")
                },
    ...
    def place_real_order(self, symbol, action, order_params, agent_name=None):
        try:
            ...
            if action == 'CLOSE':
                ...
                logger.info(
                    f"🔍 [CLOSE] 检查持仓... 目标: {target_pos_side} | 量: {raw_close_amount} | 价: {raw_close_price}")
                ...
                for pos in positions:
                    ...
                    if amt > 0:
                        ...
                        if raw_close_price > 0:
                            ...
                            if is_stop_loss:
                                logger.info(
                                    f"🛑 [CLOSE-STOP] 检测到止损场景 (现价 {current_price} -> 目标 {formatted_price})")
                                ...
                                self.exchange.create_order(symbol, order_type, close_side, final_amt, None,
                                                           params=params)

                            else:
                                logger.info(
                                    f"💰 [CLOSE-TP] 检测到止盈场景 (现价 {current_price} -> 目标 {formatted_price})")
                                order_type = 'LIMIT'
                                params['timeInForce'] = 'GTC'
                                self.exchange.create_order(symbol, order_type, close_side, final_amt,
                                                           float(formatted_price), params=params)
                            ...
                            order_type = 'MARKET'
                            logger.info(
                                f"🚀 [CLOSE-MARKET] 下单: {current_pos_side_str} -> {close_side} {formatted_amt} @ 市价")
